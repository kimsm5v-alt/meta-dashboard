...
export const buildSchoolRecordUserMessage = (input: SchoolRecordInput): string => {
  ...
  const typeGuide = TYPE_WRITING_GUIDES[round1.predictedType] || TYPE_WRITING_GUIDES['안전 균형형'];
...
## 5. 간결하고 구조적인 답변
- 마크다운 형식으로 응답하세요 (프론트엔드에서 렌더링 처리함)
  - 볼드: **텍스트**
  - 리스트: - 항목
  - 소제목: ### 제목
...
**중학교:**
- **냉소적무기력형** (35.4%): 학습 동기와 의욕 저하
- **정서조절취약형** (26.6%): 정서 관리 어려움
- **자기주도몰입형** (38.1%): 자기주도적 학습 능력 우수
