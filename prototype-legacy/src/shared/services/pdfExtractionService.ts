...
2. 1차 검사만 있으면 test1에 해당 날짜와 결과를 넣고, test2는 "미실시"로 설정하세요.
3. 2차 검사만 있으면 test1은 "미실시", test2에 해당 날짜와 결과를 넣으세요.
4. 두 검사 모두 있으면 각각 test1, test2에 넣으세요.
5. 학생 ID는 "S" + 반번호(2자리) + 출석번호(2자리) 형식 (예: S0201)
6. 반번호를 알 수 없으면 "01"로 설정하세요.
7. 출석번호를 알 수 없으면 순서대로 01, 02, 03... 으로 설정하세요.
8. 신뢰도(사회적바람직성, 반응일관성, 연속동일반응) 중 "주의" 또는 "부적절"로 표시된 항목만 reliability 배열에 넣으세요. "양호"이거나 정상이면 빈 배열 []로 두세요. 예: 사회적바람직성이 "주의"이면 ["사회적바람직성"], 모두 양호이면 [].
9. 유형명(자원소진형, 안전균형형, 몰입자원풍부형, 무기력형, 정서조절취약형, 자기주도몰입형)이 있으면 type에 넣으세요.
10. JSON만 출력하세요. 절대 마크다운 코드블록(\`\`\`)이나 설명을 포함하지 마세요.`;
