...
// 대분류별 색상 정의 (HSJ Dashboard 기준)
const DOMAIN_COLORS: Record<FactorCategory, string> = {
  '자아강점': '#00D282',       // HSJ 자아강점
  '학습디딤돌': '#4BC1FF',     // HSJ 학습 디딤돌
  '긍정적공부마음': '#67A7FF', // HSJ 긍정적 공부마음
  '학습걸림돌': '#FF849F',     // HSJ 학습 걸림돌
  '부정적공부마음': '#FF87D4', // HSJ 부정적 공부마음
};

const DOMAIN_SOFT_COLORS: Record<FactorCategory, string> = {
  '자아강점': '#DFF8EC',       // HSJ 자아강점 soft
  '학습디딤돌': '#E2F4FF',     // HSJ 학습 디딤돌 soft
  '긍정적공부마음': '#E4EFFF', // HSJ 긍정적 공부마음 soft
  '학습걸림돌': '#FFE7EC',     // HSJ 학습 걸림돌 soft
  '부정적공부마음': '#FFE6F4', // HSJ 부정적 공부마음 soft
};
...
export const StudentFactorAnalysis: React.FC<StudentFactorAnalysisProps> = ({
  ...
  useEffect(() => {
    const updateWidth = () => {
      if (containerRef.current) {
        setContainerWidth(containerRef.current.offsetWidth - 40); // padding 제외
      }
    };
    updateWidth();
  ...
  const barWidth = showCompare
    ? Math.max(Math.min(barSlotWidth * 0.95, 80), 32) // 차수 비교 시 더 넓은 막대 (2배)
            ...
            activeDomain.subCategories.forEach((subCat, subIdx) => {
              ...
              subCat.factors.forEach((factor) => {
                ...
                if (showCompare && prevT !== null) {
                  // 차수 비교 모드 - 막대 너비 확대
                  const pairWidth = Math.max(barWidth * 0.48, 28); // 최소 28px (2배)
