 ...
 * 학생 정보 저장 API 요청 타입
 */
interface SaveStudentInfoRequest {
 ...
 * 학생 정보 저장 API 응답 타입
 */
interface SaveStudentInfoResponse {
  success: boolean;
}
...
export async function saveStudentInfo(
  ...
  // TODO: 백엔드 API 연결
  // const response = await apiRequest<SaveStudentInfoResponse>(
  //   '/api/dgnss/st/student-info',
  //   {
  //     method: 'POST',
  //     body: JSON.stringify({
  //       dgnssResultId,
...
export function getStoredStudentInfo(
  ...
  // TODO: 백엔드 API에서 조회
  // const response = await apiRequest<StudentInfo>(
  //   `/api/dgnss/st/student-info?dgnssResultId=${dgnssResultId}`
  // );
