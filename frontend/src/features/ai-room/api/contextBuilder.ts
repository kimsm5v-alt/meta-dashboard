...
const formatRoundChanges = (student: Student): string | null => {
  ...
  const positive = changes.filter(
    (c) => (c.isPositive && c.diff > 0) || (!c.isPositive && c.diff < 0),
  );
  ...
  if (positive.length > 0) {
    lines.push(
      `- 긍정 변화: ${positive.map((c) => `${c.name}(${c.diff > 0 ? '+' : ''}${c.diff})`).join(', ')}`,
    );
  }
  if (negative.length > 0) {
    lines.push(
      `- 부정 변화: ${negative.map((c) => `${c.name}(${c.diff > 0 ? '+' : ''}${c.diff})`).join(', ')}`,
    );
  }
  return lines.join('\n');
};
...
const buildStudentContext = async (
  ...
  for (let index = 0; index < students.length; index++) {
    ...
    const warnings =
      assessment.reliabilityWarnings.length > 0
