...
const ColorDot = styled.span<{ $bgColor: string }>`
...
export const TypeDistributionChart: React.FC<TypeDistributionChartProps> = ({
  ...
  const showTooltip = useCallback(
    (
      ...
      bar: { data: { id: string | number; value: number | null; data: BarRecord }; color: string },
    ) => {
  ...
  return (
    <Container $height={chartHeight}>
      <ResponsiveBar
        ...
        layers={['grid', 'axes', CustomBarsLayer, 'markers', 'legends', 'annotations']}
        legends={[
          {
            dataFrom: 'keys',
            anchor: 'bottom-right',
      ...
      />
      {tooltip && (
        <TooltipBox $x={tooltip.x} $y={tooltip.y}>
          <ColorDot $bgColor={tooltip.color} />
