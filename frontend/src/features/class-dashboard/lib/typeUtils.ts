...
// 초등 유형 순서
export const TYPE_ORDER_ELEMENTARY = [
  '몰입자원 풍부형',
  '안전 균형형',
  '자원소진형',
  '미실시',
...
// 중등 유형 순서
export const TYPE_ORDER_MIDDLE = [
  '자기주도 몰입형',
  '정서조절 취약형',
  '냉소적 무기력형',
  '미실시',
...
export const TYPE_GRADIENTS: Record<string, { start: string; end: string }> = {
  ...
  // 초등 유형
  자원소진형: { start: '#F1948A', end: '#E74C3C' },
  '안전 균형형': { start: '#85C1E9', end: '#3498DB' },
  '몰입자원 풍부형': { start: '#82E0AA', end: '#2ECC71' },
  // 중등 유형
  '냉소적 무기력형': { start: '#F1948A', end: '#E74C3C' },
  '정서조절 취약형': { start: '#F8C471', end: '#F39C12' },
  '자기주도 몰입형': { start: '#82E0AA', end: '#2ECC71' },
};
...
export const abbreviateType = (type: string): string => TYPE_ABBREVIATIONS[type] || type;
 ...
 * 유형 변화 유형 결정
 */
export const getChangeType = (from: string, to: string): 'change' | 'maintain' | 'notAssessed' => {
