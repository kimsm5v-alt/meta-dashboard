...
function pickTopFactor(
  ...
  const wantHighest =
    (type === 'strength' && item.isPositive) || (type === 'weakness' && !item.isPositive);
...
function toProfileItem(item: CategoryDataItem, type: 'strength' | 'weakness'): ClassProfileItem {
  ...
  return {
    ...
    topFactorScript: findSummary(scriptsDepth3.scripts as ScriptEntry[], top.name, top.avgT),
  };
}
