...
export const INTERVENTION_SUMMARY: Record<string, string[]> = {
  ...
  초등_안전균형형: ['플래너 활용 (무엇+언제)', '시간 설계 코칭', '성공 패턴 시각화'],
  초등_몰입자원풍부형: ['도전 과제 제공', '실패→학습기회 재해석', '타인 시선 분리 연습'],
  중등_무기력형: ['정서지지 + 구체적 도움 결합', '과제 미세 분할', '작은 성공 축적'],
  중등_정서조절취약형: ['노트 필기 양식 제공', '정서인식→행동전환 훈련', '학습 전략 점검'],
  중등_자기주도몰입형: ['협력 학습 구조', '관계 기반 동기 부여', '교사-학생 관계 강화'],
};
...
export function getInterventionsForType(
  ...
  return INTERVENTIONS.filter((int) => int.school === schoolLevel && int.targetGroup === typeName);
}
...
export function getMediationPathForType(
  ...
  return MEDIATION_PATHS.find((path) => path.school === schoolLevel && path.group === typeName);
}
...
export function getMediationPathsForType(
  ...
  return MEDIATION_PATHS.filter((path) => path.school === schoolLevel && path.group === typeName);
}
...
export function getModerationEffectForType(
  ...
  const groupId = STUDENT_GROUPS.find((g) => g.school === schoolLevel && g.label === typeName)?.id;
 ...
 * 학생 유형에 따른 개입 요약 조회
 */
export function getInterventionSummary(schoolLevel: SchoolLevel, typeName: string): string[] {
...
export function getStudentGroup(
  ...
  return STUDENT_GROUPS.find((g) => g.school === schoolLevel && g.label === typeName);
}
