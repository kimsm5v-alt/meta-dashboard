...
export const API_TEACHER_ME: ApiDefinition = {
  ...
  errorCases: [{ code: 401, message: '인증 토큰이 없거나 만료됨' }],
...
export const API_TEACHER_DASHBOARD: ApiDefinition = {
  ...
  description:
    '교사의 담당 학급 목록과 각 학급별 집계 통계를 반환합니다. 프론트엔드에서 추가 계산 없이 바로 렌더링 가능.',
  priority: 'high',
  responseExample: {
    tcId: 'TC001',
    classes: [
  ...
  relatedFiles: [
    ...
    'src/features/api/useApiData.ts',
  ],
};
...
export const API_UPLOAD_LATEST: ApiDefinition = {
  ...
  description:
    '교사가 업로드한 PDF 검사 결과지 데이터를 조회합니다. AIDT/비바샘에서 받은 PDF 파일을 업로드하면 파싱된 학생 검사 결과가 저장됩니다.',
  priority: 'high',
  responseExample: {
    uploadId: 'upload-001',
    version: 1,
...
export const API_EXAM_CODE_GET: ApiDefinition = {
  ...
  errorCases: [{ code: 404, message: '등록되지 않은 코드' }],
  currentImpl: 'localStorage (exam_code_map) 사용 중',
  relatedFiles: ['src/features/exam/services/examService.ts'],
};
...
export const API_CLASS_ALL_STUDENTS: ApiDefinition = {
  ...
  description:
    '검사 제출 여부와 무관하게 학급에 소속된 전체 학생 목록을 반환합니다. 상담 일정 등록 시 학생 선택에 사용.',
  priority: 'high',
  responseExample: [
    {
      stdtId: 'a1b2c3d4e5f6',
  ...
  relatedFiles: ['src/features/schedule/components/ScheduleStudentPicker.tsx'],
};
...
export const API_COUNSELING_CREATE: ApiDefinition = {
  ...
  errorCases: [{ code: 400, message: '필수 필드 누락' }],
  currentImpl: 'mockUnifiedCounselingService (localStorage)',
