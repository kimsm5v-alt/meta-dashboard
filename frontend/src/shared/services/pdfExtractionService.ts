...
const sanitizeExtractedData = (data: Record<string, unknown>): void => {
  const classes = data.classes as
    | Record<
        ...
        {
          students?: Array<{
            test1?: { reliability?: unknown };
            test2?: { reliability?: unknown };
          }>;
        }
      >
    | undefined;
...
const validateExtractedData = (data: unknown): ValidationResult => {
  ...
  if (!data || typeof data !== 'object') {
    return {
      ...
      studentCount: 0,
      classCount: 0,
      errors: [{ type: 'parse_error', message: '유효한 JSON이 아닙니다' }],
      warnings,
    };
  }
  ...
  const classes = raw.classes as Record<
    string,
    {
      students?: Array<{
        id?: string;
        name?: string;
        test1?: { tScores?: Record<string, number> };
        test2?: { tScores?: Record<string, number> };
      }>;
    }
  >;
  let totalStudents = 0;
    ...
    for (const student of classData.students) {
      ...
      if (!student.name) {
        errors.push({
          ...
          message: `학생 이름이 없습니다 (ID: ${student.id})`,
        });
      }
      ...
      if (!tScoresData) {
        errors.push({
          type: 'no_data',
          studentId: student.id,
          message: `${student.name || student.id}: T점수 데이터가 없습니다`,
        });
        continue;
      }
      ...
      const matchedCount = normalizedFactors.filter((f) =>
        extractedKeys.some((k) => k === f),
      ).length;

      if (matchedCount < 30) {
        warnings.push({
          type: 'few_factors',
          message: `${student.name || student.id}: ${matchedCount}/38 요인만 추출됨`,
        });
      }
      ...
      for (const [factor, score] of Object.entries(tScoresData)) {
        ...
        if (score < 10 || score > 90) {
          warnings.push({
            type: 'score_out_of_typical_range',
            message: `${student.name || student.id}: ${factor} T=${score} (범위 이탈)`,
          });
        }
      }
    }
...
export const extractFromPdf = async (file: File): Promise<ExtractionResult> => {
  try {
    ...
    const response = await callGeminiMultimodal(EXTRACTION_PROMPT, {
      data: base64Data,
