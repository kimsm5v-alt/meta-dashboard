...
export const metaApi = {
  ...
  getExamList: async (): Promise<ExamListItem[]> => {
    const response = await apiRequest<{ list: ExamListItem[] }>('/etc/meta/tc/list');
  ...
  getExamDetail: async (dgnssId: number): Promise<ExamDetail> => {
    const response = await apiRequest<ExamDetail>(`/etc/meta/tc/detail?dgnssId=${dgnssId}`);
  ...
  getClassAnalysis: async (
    ...
    const response = await apiRequest<ClassAnalysisData>(`/etc/meta/tc/analysis?${params}`);
  ...
  getNeedAttentionStudents: async (dgnssId: number): Promise<unknown> => {
    const response = await apiRequest<unknown>(`/etc/meta/tc/need?dgnssId=${dgnssId}`);
    return response.resultData;
  },
  ...
  getNotSubmittedStudents: async (dgnssId: number): Promise<unknown> => {
    const response = await apiRequest<unknown>(`/etc/meta/tc/notsubm/list?dgnssId=${dgnssId}`);
    return response.resultData;
  },
};
 ...
 */
export function extractTScores(items: TScoreItem[], round: 1 | 2 = 1): number[] {
  return items
