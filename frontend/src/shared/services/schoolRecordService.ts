...
const MOCK_RESPONSES: Record<SchoolRecordCategory, string> = {
  comprehensive:
    '학습에 대한 열의가 있으며, 자기 점검 능력이 발전하고 있는 학생임. 또래 관계에서 적극적으로 소통하며, 모둠 활동에서 협력하는 자세를 보임. 학업 스트레스 관리에 관심을 기울이면 더욱 성장할 것으로 기대됨.',

  learning:
    '수업 시간에 집중력을 유지하며, 학습 목표를 설정하고 달성하기 위해 노력하는 모습을 보임. 자기 점검 습관이 형성되어 가고 있으며, 메타인지 능력 향상을 위한 꾸준한 노력이 돋보임.',

  personality:
    '긍정적인 자아상을 가지고 있으며, 새로운 도전에 대해 자신감 있게 임하는 태도를 보임. 실패를 두려워하지 않고 성장의 기회로 삼으려는 마음가짐이 인상적임.',

  socialSkills:
    '친구들과 원만하게 소통하며, 모둠 활동에서 협력적인 자세를 보임. 타인의 의견을 경청하고 자신의 생각을 적절하게 표현하는 능력이 있음.',

  selfManagement:
    '학업 과제를 계획적으로 수행하려는 노력이 보이며, 시간 관리 능력이 향상되고 있음. 스트레스 상황에서 적절한 대처 방법을 찾으려는 모습이 관찰됨.',
};
...
생활기록부에 바로 기재할 수 있는 문구를 작성해주세요.`;
    ...
    const content = response.success ? response.content : MOCK_RESPONSES[request.category];
