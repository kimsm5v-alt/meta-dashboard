...
import { transformToAssessments, transformToClass, resolveSchoolLevel } from './apiDataTransformer';
...
export interface ExamDataService {
  ...
  /** 학급 평균 T점수 조회 */
  getClassAverageTScores: (claId: string, ordNo: number) => Promise<number[]>;
}
...
export const examDataService: ExamDataService = {
  getClasses: async () => {
    try {
      ...
      for (const exam of examList) {
        ...
        const cls = transformToClass(examDetail, students, schoolLevel, grade, classNumber);
  ...
  getStudentAssessments: async (studentId: string, dgnssId: number, schoolLevel: SchoolLevel) => {
    try {
  ...
  getClassAverageTScores: async (claId: string, ordNo: number) => {
    try {
      ...
      const scores = items.filter((item) => item.DEPTH === 5).map((item) => item.tScore);
