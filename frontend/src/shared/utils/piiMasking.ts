 ...
 * @returns 치환된 텍스트
 */
export const applyStudentAliases = (text: string, aliasMap: Map<string, string>): string => {
  let result = text;

  // 긴 이름부터 먼저 치환 (부분 매칭 방지)
  const sortedNames = Array.from(aliasMap.keys()).sort((a, b) => b.length - a.length);
 ...
 * @returns 복원된 텍스트
 */
export const restoreStudentNames = (text: string, aliasMap: Map<string, string>): string => {
  let result = text;
 ...
 * @returns 마스킹 결과
 */
export const maskAll = (text: string, studentNames?: string[]): MaskingResult => {
