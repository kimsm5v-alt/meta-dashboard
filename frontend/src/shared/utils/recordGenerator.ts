...
const calculateSubCategoryScores = (tScores: number[]): Record<SubCategory, number> => {
 ...
 */
export const getTopStrengthsFromAssessment = (assessment: Assessment): StrengthItem[] => {
...
export const getRecommendedSentences = (
  ...
  return EXAMPLE_SENTENCES.filter(
    (sentence) =>
      strengthNames.includes(sentence.subCategory) && sentence.schoolLevel === schoolLevel,
  );
};
...
export const getSentencesForSubCategory = (
  ...
  return EXAMPLE_SENTENCES.filter(
    (sentence) => sentence.subCategory === subCategory && sentence.schoolLevel === schoolLevel,
  );
};
...
export const calculateCategoryScores = (tScores: number[]): Record<FactorCategory, number> => {
  const subCategoryScores = calculateSubCategoryScores(tScores);
 ...
 */
export const toSchoolLevelKr = (schoolLevel: '초등' | '중등' | '고등', grade?: number): SchoolLevelKr => {
 ...
 */
export const schoolLevelTypeToKr = (type: 'ELEMENTARY' | 'MIDDLE' | 'HIGH'): SchoolLevelKr => {
  switch (type) {
    case 'ELEMENTARY':
      return '초등';
    case 'MIDDLE':
