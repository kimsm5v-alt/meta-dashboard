...
interface CaptureState {
  ...
  overlayOpen: boolean;
  /** 컴포저에 첨부 대기 중인 캡처 이미지(data URI). 턴 한정 데이터라 persist하지 않음 */
   ...
   * (예: 학생 대시보드의 DataHelperChatbot). 0보다 크면 FloatingAssistant 버블이
  ...
  /** 로그아웃 등 계정 전환 시 호출 — 이전 사용자의 대기 중인 캡처 이미지가
   * 새 세션으로 넘어가지 않도록 전부 초기화한다. */
  reset: () => void;
}
...
export const useCaptureStore = create<CaptureState>()((set) => ({
  ...
  registerBottomRightFab: () => set((s) => ({ bottomRightFabCount: s.bottomRightFabCount + 1 })),
