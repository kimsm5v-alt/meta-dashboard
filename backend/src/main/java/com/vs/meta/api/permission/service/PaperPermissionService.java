...
import com.vs.meta.api.permission.dto.PaperPermissionResponse;
...
public class PaperPermissionService {
    ...
    /** 계정 권한 조회. 행 없으면 기본값(종합 Y, 자기조절 N) 적용. */
    @Transactional(readOnly = true)
    public PaperPermissionResponse getPermissions(long userNo) {
        ...
        return new PaperPermissionResponse(comprehensive, selfreg);
    }

    /** 해당 paperIdx 검사 허용 여부. (Phase 2 강제 지점에서 사용) */
    @Transactional(readOnly = true)
    public boolean isAllowed(long userNo, String paperIdx) {
        PaperPermissionResponse perm = getPermissions(userNo);
        if (PAPER_COMPREHENSIVE.equals(paperIdx)) {
            return perm.comprehensive();
        }
